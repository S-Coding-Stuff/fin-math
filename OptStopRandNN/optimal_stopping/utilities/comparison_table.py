...
from optimal_stopping.utilities.absl_compat import flags
...
ALGOS_ORDER = [
  ...
  "RLSMElu", "RLSMSilu", "RLSMGelu", "RLSMSoftplus", "RLSMSoftplusReinit",
  "RRLSM", "RRLSMmix",
  "EOP", "B", "Trinomial",
]
...
def _write_table_for(
  ...
  if "EOP" in all_algos:
    ...
    ref_algo = None
  for a,b in [["LSM", "RLSM"], ["DOS", "RLSM"], ["NLSM", "RLSM"],
  ...
  # bold_algos = ["RLSM", "RRLSM"]
  bold_algos = []
  table_dir = Path(__file__).resolve().parent / "../../../latex/tables_draft"
  table_dir.mkdir(parents=True, exist_ok=True)
  table_path = (table_dir / f"{label}.tex").resolve()
  ...
  with table_path.open("w") as tablef:
