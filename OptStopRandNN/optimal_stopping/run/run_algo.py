...
from optimal_stopping.utilities.absl_compat import app, flags
...
except ModuleNotFoundError:  # pragma: no cover - optional tree baselines
    binomial = None
    trinomial = None
...
flags.DEFINE_enum(
    ...
    "Path sampling engine for Black-Scholes models.",
)
flags.DEFINE_list(
    "path_samplers",
    None,
    "Optional list of path samplers to sweep in one run. "
    "Overrides --path_sampler when provided.",
)
...
_CSV_HEADERS = ['algo', 'model', 'path_sampler', 'payoff', 'drift', 'volatility', 'mean',
...
if binomial is not None:
    _ALGOS["B"] = binomial.BinomialPricer
if trinomial is not None:
    _ALGOS["Trinomial"] = trinomial.TrinomialPricer
...
def _run_algos():
  ...
  delayed_jobs = []
  selected_path_samplers = (
      tuple(FLAGS.path_samplers)
      if FLAGS.path_samplers
      else None
  )
  ...
  for config_name, config in configs_getter.get_configs():
    ...
    path_samplers = selected_path_samplers
    if path_samplers is None:
      path_samplers = tuple(getattr(config, "path_samplers", (FLAGS.path_sampler,)))
    combinations = list(itertools.product(
        ...
        config.ridge_coeff, path_samplers,
...
def _run_algo(
        ...
        factors=(1.,1.,1.), ridge_coeff=1., path_sampler="mc",
  ...
  Args:
   ...
   mean (float): reserved historical parameter, unused for maintained models.
   speed (float): reserved historical parameter, unused for maintained models.
   correlation (float): reserved historical parameter, unused for maintained models.
   hurst (float): in (0,1) the hurst parameter for the
            FractionalBrownianMotion and FractionalBrownianMotionPathDep stock
            models.
   nb_epochs (int): number of epochs for SGD-based or repeated-fit methods.
  ...
  if stock_model not in _STOCK_MODELS:
    raise ValueError(
        f"Unsupported stock model '{stock_model}'. "
        f"Supported models: {sorted(_STOCK_MODELS)}."
    )
  print(algo, stock_model, path_sampler, spot, volatility, maturity, nb_paths, '... ', end="")
  ...
  stock_model_ = _STOCK_MODELS[stock_model](
      ...
      maturity=maturity, path_sampler=path_sampler,
      sampler_seed=path_gen_seed)
  if algo in ['NLSM']:
      ...
      pricer = _ALGOS[algo](stock_model_, payoff_, nb_epochs=nb_epochs,
                            ...
                            train_ITM_only=train_ITM_only,
                            use_payoff_as_input=use_payoff_as_input)
  elif algo in ["RLSMRidge", "RRLSMRidge"]:
      pricer = _ALGOS[algo](stock_model_, payoff_, nb_epochs=nb_epochs,
                            hidden_size=hidden_size, factors=factors,
                            train_ITM_only=train_ITM_only,
  ...
  metrics_['path_sampler'] = path_sampler
